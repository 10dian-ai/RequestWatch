...
(() => {
  ...
  const titles = { traffic: '网络流量', pending: '拦截队列', rules: '拦截规则', containers: 'Docker 容器', guide: '接入指南' };
  ...
  async function api(path, options = {}) {
    ...
    const timeout = setTimeout(() => controller.abort(), isReplay ? 60000 : 15000);
    ...
    try {
      ...
      return response.status === 204 ? null : options.download ? response : await response.json();
    } catch (error) {
  ...
  function formatHex(value) {
    ...
    for (let offset = 0; offset < clean.length; offset += 32) {
      ...
      lines.push(`${(offset / 2).toString(16).padStart(6, '0')}  ${chunk.join(' ').padEnd(47)}  ${ascii}`);
    }
    return lines.join('\n');
  }
  function renderDetail(reset = false) {
    ...
    if (record.request_truncated || record.response_truncated || record.truncated) alerts.push('内容已截断。当前展示及搜索仅覆盖已保留内容；编辑重发可能受到限制。');
    ...
    $('edit-button').disabled = s.actionBusy || !s.detailLoaded || record.state === 'resolving';
  ...
  function renderDetailBody() {
    ...
    if (s.tab === 'overview') {
      ...
      if (record.source === 'http') { addMetadata(list, '响应状态', record.status_code); addMetadata(list, '总耗时', record.duration_ms !== null && record.duration_ms !== undefined ? `${Number(record.duration_ms).toFixed(0)} ms` : '等待响应'); }
      ...
      if (record.source === 'http') {
        ...
        addCode(target, '请求正文', record.request_body_text);
        if ((record.request_body_text === null || record.request_body_binary || record.body_binary)) target.append(element('p', '正文无法以文本显示。未修改正文时保留原始字节。', 'detail-note'));
      } else {
        addCode(target, '载荷 · 文本视图', record.payload_text);
        target.append(element('p', '单包内容不代表完整请求。二进制及加密流量请查看 HEX；非 HTTP 流量不进行应用协议重组。', 'detail-note'));
      }
      ...
      else if (!record.status_code && !record.response_headers?.length && !record.response_body_text) target.append(element('p', record.state === 'pending' ? '请求正在等待放行，尚无响应。' : record.state === 'dropped' ? '请求已丢弃，没有响应。' : '尚未收到响应，或当前记录没有响应内容。', 'detail-note'));
      else { addCode(target, 'HTTP 状态', record.status_code); addCode(target, '响应头', headerText(record.response_headers)); addCode(target, '响应正文', record.response_body_text); }
    } else {
      const hex = requestBytes(record);
      addCode(target, 'PAYLOAD · HEX / ASCII', formatHex(hex.slice(0, 131072)), 'hex');
      if (hex.length > 131072) target.append(element('p', 'HEX 预览显示前 64 KB；导出 JSON 可查看后端保留的完整字节。', 'detail-note'));
      target.append(element('p', record.source === 'http' ? '此处显示请求正文的字节视图。' : '此处显示传输层载荷；导出 JSON 可查看后端保留的原始包数据。', 'detail-note'));
    }
  ...
  function openEditor() {
    ...
    const record = s.selected;
    s.draftOriginal = { method: record.method || 'GET', url: record.url || '', headers: JSON.stringify(record.request_headers || [], null, 2), body: record.request_body_text ?? '', hex: record.payload_hex || '' };
    ...
    $('body-edit-help').textContent = (record.request_body_text === null || record.request_body_binary || record.body_binary) ? '二进制正文：未编辑时保留原始字节。填写文本将替换整个请求正文。' : '未修改的正文将保持原始字节。修改的文本由服务端重新编码。';
  ...
  function openRule(rule = {}) {
    ...
    $('rule-container').value = rule.container_id || ''; $('rule-host').value = rule.host || ''; $('rule-port').value = rule.port || ''; $('rule-keyword').value = rule.keyword || ''; $('rule-timeout').value = rule.timeout_seconds || 30; $('rule-enabled').checked = rule.enabled !== false;
  ...
  document.querySelectorAll('[data-tab]').forEach((button) => button.addEventListener('click', () => { s.tab = button.dataset.tab; renderDetailBody(); }));
