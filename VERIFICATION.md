...
日期：2026-09-06。版本：0.5.0。开发机：Windows，Python 3.14.7；目标部署：Ubuntu 24.04+，Python 3.12+。

## 0.5.0 New API 双链路、Prompt 与性能验证

- 全量 Python 回归：635 passed、9 skipped、4 subtests passed，开启 `RW_RUN_APP_INTEGRATION=1` 并指定独立 mitmdump。独立代理环境另外执行 66 项全部通过，含实际 HTTP/HTTPS、CA 信任、流式响应和完整正文；这些与全量回归部分重叠，不相加。两个上游依赖弃用警告；Windows 不适用的内核/权限场景仍未执行。
- 新增实际双代理集成：客户端 → 反向 mitmdump → 本地模拟 New API 中转 → 正向 mitmdump → 本地供应商。约 1.8 MB 中文请求，客户端与上游各自原始正文 SHA 一致；中转修改模型名、附加 system prompt，验证两段保存分离、Bearer/Host 保留、链路筛选及完整 HTTP 下载。两边首段在源站结束前可读，最终 SSE 字节与客户端收到的内容一致，Prompt/思考内容/尾部和解析下载一致。此测试使用本地中转夹具，不代表已在用户的真实 New API 容器上验证。
- 默认 New API 模式不启动 Scapy/NFQUEUE；只有填写 New API 地址并启用代理时启动反向入口、检查反向端口冲突并添加保护端口。旧配置使用 8081、关闭代理或通用网络模式不会因未启用的反向入口冲突。面板检测反向端口被占用时拒绝应用，原设置仍可访问。显式自指地址被拒绝；任意 DNS 别名的循环仍需部署时避免。
- Prompt 提取 30 项回归：OpenAI Chat/Responses、Anthropic、Gemini 的请求、响应、推理与工具参数；UTF-8 跨块、未完成事件、未知/混合格式、超大 JSON/SSE 事件均保留完整原文并提示。Prompt 与自动解析缓存按格式/方向隔离，原始捕获不改写。20,001 个 SSE 事件（1,488,904 B）提取至尾部，Python tracemalloc 峰值 1,525,479 B；此为本地合成资源检查。
- 查询使用轻量索引，读取列表、统计、活动过期及 GC 引用无需解析完整记录。事务测试涵盖批计数、重复 ID、淘汰、失败回滚、并发和旧库迁移；关键词仍匹配完整正文末尾，链路筛选只读取真实标记。100,000 条本地合成记录：HTTP 列表中位数 50.421 → 0.669 ms，容器列表 352.484 → 66.337 ms，全文末尾搜索 872.591 → 196.837 ms。代价是首次索引迁移约 10.1 秒、额外索引空间；4,000 包纯 Store 写入 125.873 → 201.905 ms。该权衡面向高频读取，不宣称所有操作都变快。
- SSE 原始字节持续落盘；首段及时发布，中间快照按增长量 1–5 秒合并、EOF 立即发布。90 秒等效增长负载（无 sleep）复制字节 125,617,036 → 13,102,022，减少 89.57%；最终 1,350,758 字节 SHA 相同。不可变快照仍有重复 I/O，此优化没有消除其理论增长项，也不代表 Ubuntu 线速。
- 浏览器性能回归：约 2.08 MB 请求 + 2.38 MB 响应，连续三轮轮询无正文重复下载/节点重建；一侧响应更新不重绘另一侧，8,000 个 ACK 元数据变化不重建 TCP 正文。大正文以 1,024 字符独立块排版，所有字符均保留；首屏本机约 1.35 秒，全文搜索可跨块定位末尾，严格 scrollTop 不漂移。
- 新 ui-newapi 通过：两段标签和未知历史、默认 Prompt/原文独立切换与缓存、单侧流更新、原文精确下载、TCP 筛选清理冲突、设置保存应用、Docker 单一候选填草稿与歧义保留、移动布局。Docker 端口发现保存发布地址/端口，并保护返回对象不被界面修改。
- 实际 7030 演示实例确认中文 system/user prompt、正文与思考内容在双栏完整显示；1440×900 和 390×844 无页面横向溢出、无脚本错误。演示记录明确标记，不是真实用户网络捕获。
- 最终浏览器回归 ui-body-workspace、ui-readable、ui-packet-details、ui-newapi、ui-performance 全部通过；独立 8044 完整 ui-smoke 通过，覆盖 2 MiB 请求/3 MiB 响应末尾、精确下载、HEX、编辑重发、拦截决定、所有设置与移动布局。该 full smoke 未传 TCP fixture，TCP 由专用 UI/后端测试覆盖。
- Python compileall、JavaScript 语法、git diff --check、0.5.0 wheel 构建及内容比对通过，包内版本与 AGPL 元数据正确。
